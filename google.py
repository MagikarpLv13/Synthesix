...
def _req(term, results, lang, start, proxies, timeout, safe, ssl_verify, region):
    resp = get(
        ...
        params={
            ...
            "safe": safe,
            "gl": region,
            "udm": 14,           # Disable IA content
            "nfpr": 1,           # Disable personalized results
            "filter": 0,
        },
        proxies=proxies,
        timeout=timeout,
    ...
    try:
        ...
        # Ouvre le navigateur pour voir la page
        # Récupère le navigateur par défaut de l'utilisateur
        browser = webbrowser.get()
        # Ouvre la page dans le navigateur
        # url + paramètres
        browser.open("https://www.google.com/search?q="+term+"&num=15&start=0&filter=0&nfpr=1&udm=14")
...
def google_search(query, num_results=15):
    ...
    results = []
    for j in search(query, num_results=num_results, unique=True, safe="off", advanced=True):
        results.append({
            "title": j.title,
            "link": j.url,

...
def _coordinates(entity: Mapping) -> tuple[float | None, float | None]:
    if entity.get("entity_type") != "coordinates":
        return None, None
...
def build_export_graph(
    ...
    *,
    ...
    investigation_node = GraphNode(
        ...
        label=f"Investigation: {investigation.get('title', investigation_id)}",
    ...
    for entity in included_entities:
        ...
        entity_node = GraphNode(
            ...
            tags=_tags(
                _entity_tags(str(entity.get("entity_type", "") or "")),
                statuses,
            ),
            confidence=max(
...
def _native_properties(
    ...
    *,
    ...
    for key, value in properties.items():
        ...
        result.append(
            {
                "key": names.get(key, key.replace("_", " ").title()),
                "value": value,
                "type": _native_property_type(key, value),
            }
        )
    return result
